...
export const restSupport = async (req, res) => {
    try {
        const resData = req.restaurantData;
        req.body.userId = resData._id;
        let ticketId;
        let existingTicket;
        ...
        await commonService.create(SUPPORT_MODEL, { ...req.body, service: 5 });
...
export const restSupportlist = async (req, res) => {
    try {
        let { page = 1, limit = 10 } = req.query;
        limit = parseInt(limit);
        page = parseInt(page);
        let skipIndex = (page - 1) * limit;
        let params = { service: 5, userId: String(req.restaurantData?._id) };
