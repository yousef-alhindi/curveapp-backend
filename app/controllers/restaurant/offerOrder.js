...
export const OfferSaveTransaction = async (req, res) => {
    try {
        ...
        const transaction = await RestaurantTransactionModel.create({
            paymentFor: 3,
            userId: null,
        ...
        if (req.body?.isWalletSelected) {
            let wallet = await RestaurantWalletModel.findOne({ restaurantId: new mongoose.Types.ObjectId(req.restaurantData?._id) });
            if (!!wallet) {
                let newBalance = 0
