...
export const supplementSupport = async (req, res) => {
    try {
        const supData = req.supplementData;
        req.body.userId = supData._id;
        if (!req.body.service) {
...
export const supplementSupportlist = async (req, res) => {
    try {
        let { page = 1, limit = 10, service } = req.query;
        limit = parseInt(limit);
        page = parseInt(page);
        let skipIndex = (page - 1) * limit;
        let params = { service: service, userId: String(req.supplementData?._id) };
