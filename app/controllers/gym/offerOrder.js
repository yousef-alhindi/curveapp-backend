...
import { Gym_Offer_Category_Model } from '../../models/gym/offerCategory.model';
...
export const OfferSaveTransaction = async (req, res) => {
    try {
        ...
        const alreadyPurchased = await commonService.findOne(Gym_Offer_Order_Model, { offerId: new ObjectId(req.body.offerId), gymId: gymData._id })
        if (!!alreadyPurchased) {
        ...
        req.body.gymId = gymData._id
        let transactionId = '#' + randomize('0', 9);
        const transaction = await GymTransactionModel.create({
            paymentFor: 3,
            userId: null,
        ...
        let data = await commonService.create(Gym_Offer_Order_Model, req.body)
        
        if (req.body?.isWalletSelected) {
            let wallet = await GymWalletModel.findOne({ gymId: new mongoose.Types.ObjectId(req.gymData?._id) });
            if (!!wallet) {
                let newBalance = 0
                ...
                await GymWalletModel.findByIdAndUpdate(wallet._id, { balance: newBalance });
            }
        }
...
export const newOfferList = async (req, res) => {
    try {
        ...
        if (status == "1") {
            ...
            let existingData = await Gym_Offer_Order_Model.aggregate([{
                $match: {
                    isDeleted: false,
                    gymId: gymData._id,
            ...
            let existingData = await Gym_Offer_Category_Model.find({ gymId: gymData._id }).populate("categoryId").sort({ createdAt: -1 })
...
export const buyOfferList = async (req, res) => {
    try {
        ...
        if (status === "1") {
            ...
            detail = await Gym_Offer_Category_Model.find({ gymId: gymData._id })
                .populate("categoryId")
                .sort({ createdAt: 1 });
        }
...
export const offerActiveandDeactive = async (req, res) => {
    try {
        ...
        let joinedOfferResponse = await commonService.findById(Gym_Offer_Order_Model, joinedOfferId);
        
        if (!joinedOfferResponse) {
            return sendErrorResponse(res, "Joined Offer ID is invalid", HttpStatus.BAD_REQUEST);
        }

        let resp = await Gym_Offer_Order_Model.findByIdAndUpdate(
            { _id: joinedOfferId },
...
export const offerSaveCategory = async (req, res) => {
    try {
        ...
        let offerCategoryResponse = await commonService.findOne(Gym_Offer_Category_Model, { categoryId, gymId: req.gymData?._id })
        ...
        let resp = await Gym_Offer_Category_Model.create({ categoryId, transactionId: transaction._id, orderId, amount, gymId: req.gymData?._id, createdAt: new Date().getTime() });
...
export const categoryActiveandDeactive = async (req, res) => {
    try {
        ...
        let joinedOfferResponse = await commonService.findById(Gym_Offer_Category_Model, joinedCategoryId)
        if (!joinedOfferResponse) {
            return sendErrorResponse(res, "Joined Category ID is invalid", HttpStatus.BAD_REQUEST);
        }

        let resp = await Gym_Offer_Category_Model.findByIdAndUpdate(
